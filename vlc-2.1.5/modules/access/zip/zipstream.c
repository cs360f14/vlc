...
static node* findOrCreateParentNode( node *root, const char *fullpath )
{
	// comments said the function is static because the implementation differs
	// since this .c file creates a playlist from a zip file, it makes sense that
	// until the directory pathway is null terminated, this function will continue to
	// run, making the pathways and directories to allow the playlist to exist after
	// unzipping.

	// zipfile strings -> actual playlist (what this function does)

	// a node data type contains
	// root -> child (a node pointer)
	// root -> media (an item pointer)
	// root -> name (a char pointer. ie, a string)
	// root -> next (a note pointer to the next node)
	// -assuming structure is of a top layer with linear roots (next) with a connection
	// to a lower level (child) where each node has a item pointer and a name.

	// media -> id (an int)
	// media -> next (an item pointer)

    char *folder;
    char *path = strdup( fullpath ); //Allocate memory by copying the string
    								 // within fullpath and returning the pointer
    								 // to the allocated memory.

    folder = path;					 // assign both folder and path to point to the
    								 // same piece of memory

    assert( root );					 // checks to see if root is not
    								 // equal to zero. (testing)

    char *sep = strchr( folder, '/' ); // searches the string fullpath for the
    								   // character '/' and returns a pointer
    								   // to the first occurence of the '/'
    								   // or returns NULL if not found.

    if( !sep )    	  				 // if sep points to a NULL
    								 // ie. sep = 0

    {
        free( path );   // frees the copied string dynamic space pointed at by folder
        return root;	// returns the root for this as the parent node

    }

    *sep = '\0'; // assign the '/' character the string termination character

    ++sep;		// moves the value of the pointer down one, it is now
    			// pointing at the character after the '/', which has been
    			// replaced by the '\0'

    node *current = root->child; // has the new node pointer current
    							 // point to the level below node.


    while( current ) // current != 0 or NULL
    {
        if( !strcmp( current->name, folder ) ) // taking true branch (folder)
        									   // if the directory names are equal
        									   // ie. returns a 0
        {
            /* We found the folder, go recursively deeper */
            node * parentNode = findOrCreateParentNode( current, sep );
            free (path);
            return parentNode;				 // returns the node that has the same name
            								 // as the directory we are looking for
            								 // or a (or a new node)
        }
        current = current->next;   			 // directory names did not match
        									 // try the next horizontal level
        									 // directory
    }

    /* If we are here, then it means that we did not find the parent */
    // only continues if the folder is not already a null terminator.
    // ie. there should be more of the directory but there isin't.
    node *ret = new_node( folder );			// creates a new node pointer
    										// via the new_node helper function

    //new_node - accepts a char pointer
    //		   - allocates a block of zero-initiated memory via calloc the
    //			 size of a node
    //		   - converts the node name via convert_xml_special_chars helper function
    //				this helper function is gnarly. XML-encode an UTF-8 string
    //				param str nul-terminated UTF-8 byte sequence to XML-encode
    //				return XML encoded string or NULL on error

    if( !root->child )						// if root's child pointer is NULL
        root->child = ret;					// assign it to the newly created node
    else									// otherwise
    {
        current = root->child;				// assign current to point to root's child
        while( current->next )				// while current (ie. root) has horizontal
        									// directories
        {
            current = current->next;		// assign current to those directories
        }
        current->next = ret;				// when current's next == NULL
    }

    /* And now, create the subfolders */ // for the rest of the pathway that should be there
    									 // but isin't.
    ret = findOrCreateParentNode( ret, sep );

    free( path );							// frees the dynamically allocated memory
    return ret;								// returns the pathway with all folders
}
